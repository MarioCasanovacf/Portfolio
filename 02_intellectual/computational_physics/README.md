...
| Layer | Technology |
|---|---|
| Compute | `numpy` + `scipy.sparse` + `scipy.sparse.linalg` (sparse tridiagonal factorization) + `numba` (`@jit` Velocity-Verlet) |
| Synthetic data | in-house generator (`src/data_generator.py`) |
| Visualization | `matplotlib` (animations, probability contours) + `seaborn` (angle histogram) |
