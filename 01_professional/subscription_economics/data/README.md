...
### `subscriptions.csv`
Subscription fact table. Each row is one SaaS subscription tied to a user. Conversion probability depends on device type and onboarding variant. Churn is simulated as a **constant monthly hazard** (a geometric survival process; Camera 0.040, Doorbell 0.075, Variant onboarding ×0.90), so subscriber lifetimes are geometric and the LTV identity is exactly recoverable. Subscriptions still active at the warehouse snapshot (2023-05-31) are right-censored (empty `SubscriptionEnd`).
