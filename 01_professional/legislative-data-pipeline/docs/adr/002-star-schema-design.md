...
**Star schema** with a dated version history (SCD Type 2) on `dim_legislator`.
...
3. **SCD requirement:** Caucus switches (party changes) are politically
   ...
   schema + a dated version-history dimension (SCD2) handles this cleanly.
...
- `dim_legislator` includes dated version-history fields: `effective_from`, `effective_to`,
  `is_current`, `_hash`.
