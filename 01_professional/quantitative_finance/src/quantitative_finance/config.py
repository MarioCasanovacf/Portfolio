...
class Settings(BaseSettings):
    ...
    lob_impact: float = Field(
        default=0.00012,
        ge=0,
        description=(
            "Permanent price-impact coefficient: each aggressive execution nudges "
            "the latent mid by lob_impact * executed_size in the aggressor's "
            "direction, planting a recoverable order-flow -> price (Kyle lambda) link."
        ),
    )
